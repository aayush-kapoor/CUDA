...
// Kernel for uniform random initialization
__global__ void uniform_init_kernel(float* data, 
    ...
    if (idx < size) {
        // Simple random number generation on GPU
...
// Kernel for normal distribution initialization
...
void XavierInitializer::initialize(Tensor<float>* tensor) {
    ...
    const int fan_in = shape[shape.size() - 2];  // input features
    const int fan_out = shape[shape.size() - 1]; // output features
